...
public class AiChatController {
    ...
    public ResponseEntity<DataResponseDTO<String>> aiCreateMinutes(@RequestBody AiMinutesCreateRequestDTO aiMinutesCreateRequestDTO) {
        ...
        return ResponseEntity.ok(DataResponseDTO.of("AI 비서 - 회의록 작성이 완료되었습니다."));
    }

    @PostMapping("/board/minutes/message")
    public ResponseEntity<DataResponseDTO<String>> aiGetMessagesForMinutes(@RequestBody AiGetMessagesForMinutesRequestDTO aiGetMessagesForMinutesRequestDTO) {

        log.info("요청자 : Flask Server, AI Get Messages for Minutes -> start");
        String contents = aiChatService.aiGetMessagesForMinutes(aiGetMessagesForMinutesRequestDTO);

        log.info("요청자 : Flask Server, AI Get Messages for Minutes -> success");
        return ResponseEntity.ok(DataResponseDTO.of(contents, "AI 비서 - 회의록 작성에 필요한 메시지 내역 전달이 완료되었습니다."));
    }
}
